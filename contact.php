...
if(isset($_POST['delete'])){
   ...
   $verify_delete = $conn->prepare("SELECT * FROM `msg` WHERE id = ?");
   $verify_delete->execute([$delete_id]);

   if($verify_delete->rowCount() > 0){
      $delete_bookings = $conn->prepare("DELETE FROM `msg` WHERE id = ?");
...
<section class="grid">
   ...
   <?php
      $select_messages = $conn->prepare("SELECT * FROM `msg`");
   ...
   <div class="box" style="text-align: center;">
      ...
      <a href="dash.php" class="btn">go to home</a>
   </div>
   <?php
      }
   ?>
